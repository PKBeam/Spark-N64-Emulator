...
export {
    ...
    struct RSP_DMA_SPADDR : WriteableRegister {
    ...
    struct RSP_DMA_RAMADDR : WriteableRegister {
        enum class MEM_BANK : bool {
            DMEM = 0,
            IMEM = 1,
        };
    ...
    struct RSP_DMA_RDLEN : WriteableRegister {
        uint32_t            : 3;
    ...
    struct RSP_DMA_WRLEN : WriteableRegister {
        uint32_t            : 3;
    ...
    struct RSP_STATUS : WriteableRegister {
        uint32_t halted   : 1;
        uint32_t broke    : 1;
        uint32_t dmaBusy  : 1;
        uint32_t dmaFull  : 1;
    ...
    struct RSP_PC : WriteableRegister {
        uint32_t         : 2;
        uint32_t pc_11_2 : 10;
        uint32_t         : 20;
    ...
    struct rspRegs {
        RSP_DMA_SPADDR  dmaSpAddr;
        RSP_DMA_RAMADDR dmaRamAddr;
        RSP_DMA_RDLEN   dmaRdLen;
        RSP_DMA_WRLEN   dmaWrLen;
        RSP_STATUS      status;
        RSP_DMA_FULL    dmaFull;
        RSP_DMA_BUSY    dmaBusy;
        RSP_SEMAPHORE   semaphore;
        RSP_PC          pc;
    };
}
